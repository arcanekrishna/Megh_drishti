...
class AutomatedAlertEngine:
    ...
    def generate_alerts(
        self,
        predictions:       Dict[str, np.ndarray],   # {'thunderstorm': (T,H,W), ...}
        ...
        """
        Scans all nowcast lead-time frames and generates categorized alerts
        for each monitored vulnerable zone.
        """
        ...
        for zone in CRITICAL_VULNERABLE_ZONES:
            ...
            max_cb_prob = max_ts_prob = max_ff_prob = 0.0
            critical_step = 0
            ...
            for t in range(n_steps):
                ...
                if p_cb > max_cb_prob:
                    max_cb_prob   = p_cb
                    critical_step = t
                max_ts_prob = max(max_ts_prob, p_ts)
                max_ff_prob = max(max_ff_prob, p_ff)
            ...
            if overall_max >= self.yellow_threshold:
                ...
                if max_cb_prob >= max_ts_prob and max_cb_prob >= max_ff_prob:
                    ...
                    event_description = (
                        "High-velocity convective rainburst with extreme rainfall rate "
                        "(>100 mm/hr) potential in the catchment area."
                    )
                elif max_ff_prob >= max_ts_prob:
                    primary_event     = "FLASH_FLOOD"
                    event_description = (
                        "Severe hydrological surge and valley drainage inundation "
                        "fueled by upstream cloudburst and steep terrain funneling."
                    )
                else:
                    primary_event     = "SEVERE_THUNDERSTORM"
                    event_description = (
                        "Violent convective storm with intense vertical updrafts, "
                        "lightning, and high wind gusts expected."
                    )

                lead_time_h    = lead_times_hours[min(critical_step, len(lead_times_hours) - 1)]
                impact_eta     = reference_time + timedelta(hours=lead_time_h)
                countdown_min  = round(lead_time_h * 60)
                ...
                alert = {
                    ...
                    "impact_countdown_minutes": countdown_min,
                    "estimated_time_to_impact": impact_eta.strftime("%Y-%m-%d %H:%M IST"),
                    ...
                    "expires_at":    (reference_time + timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S IST"),
                }
                alerts.append(alert)

        # Sort: RED first → then by peak probability descending
        severity_order = {"RED": 0, "ORANGE": 1, "YELLOW": 2}
        alerts.sort(key=lambda a: (severity_order.get(a["severity"], 3), -a["peak_probability"]))
        return alerts

    def _prescribe_sop(
        self, severity: str, event_type: str, region: str
    ) -> List[str]:
        """Standard Operating Protocol checklists for District Disaster Management Authorities."""
        if severity == "RED":
            return [
                ...
                "💧 DAM & BARRAGE OPS: Issue pre-release alerts to downstream hydel project authorities.",
            ]
        elif severity == "ORANGE":
            return [
                "⚠️ CIVIC ALERT: Restrict public movement near mountain streams and flood-prone drainage nullahs.",
...
if __name__ == "__main__":
    ...
    lats   = np.linspace(29.5, 32.5, 13)
    lons   = np.linspace(76.0, 80.0, 17)
    sample = {
        "thunderstorm": np.random.uniform(0.1, 0.9, size=(6, 13, 17)),
        "cloudburst":   np.random.uniform(0.1, 0.95, size=(6, 13, 17)),
        "flash_flood":  np.random.uniform(0.1, 0.9, size=(6, 13, 17)),
    }
    alerts = engine.generate_alerts(sample, [0.5, 1.0, 1.5, 2.0, 2.5, 3.0], lats, lons)
